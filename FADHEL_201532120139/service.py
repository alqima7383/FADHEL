...
def comment_delete(commentID):
    # get goal comment
    db_tool.delete_comment_relative(commentID)
    return jsonify({'delete': "Done"})

@app.route('/article_delete/<articleID>')
def article_delete(articleID):
    # get goal comment
    db_tool.delete_article_relative(articleID)
    message = 'You are using inappropriate words!'
...
def post_article():
    if request.method == 'POST':
        ...
        if Tool.check_short_time() == 'pivot':
            ...
            inappropriate_list = []
            for line in f:
                if line.strip() in title or line.strip() in highlight or line.strip() in abstract :
                    inappropriate = True 
                #inappropriate_list.push(line.strip())
            if inappropriate:
                message = 'You are using inappropriate words!'
                return render_template('error.html', message=message)
            time = datetime.now()
